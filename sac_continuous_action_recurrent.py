...
class Actor(nn.Module):
    ...
    def forward(self, x, seq_lengths, hidden_in=None):
        x = F.relu(self.fc1(x))

        x = pack_padded_sequence(x, seq_lengths, enforce_sorted=False)
        self.lstm1.flatten_parameters()
        x, hidden_out = self.lstm1(x, hidden_in)
        x, x_unpacked_len = pad_packed_sequence(x)
    ...
    def get_action(self, x, seq_lengths, hidden_in=None):
        mean, log_std, hidden_out = self(x, seq_lengths, hidden_in)
        std = log_std.exp()
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            (
                ...
                rewards,
                seq_lengths,
            ) = rb.sample_history(args.batch_size, history_length=4)
            ...
            with torch.no_grad():
                ...
                qf1_next_target = qf1_target(
                    next_observations, next_state_actions, seq_lengths
                )
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    pi, log_pi, _, _ = actor.get_action(observations, seq_lengths)
                    qf1_pi = qf1(observations, pi, seq_lengths)
